...
const getDepartmentOfficers = async (req, res) => {
  try {
...
const departmentLogin = async (req, res) => {
  try {
    ...
    if (role === "main") {
      const mainOfficer = await Officer.findById(
        nagar.mainOfficer
      );

      if (
        mainOfficer &&
        mainOfficer.email === email &&
        mainOfficer.password === password
      ) {
        return res.json({
    ...
    if (!department || !role) {
      return res.status(400).json({
        message: "department and role required",
      });
    }

    if (role === "head") {
      const deptHead = await Officer.findById(
        nagar[department].head
      );

      if (
        deptHead &&
        deptHead.email === email &&
        deptHead.password === password
      ) {
        return res.json({
          message: "login successful",
          success: true,
