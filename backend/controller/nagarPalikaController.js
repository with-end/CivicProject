...
const getAllNagarPalikas = async (req, res) => {
  try {
    const nagarpalikas = await NagarPalika.find()
      .populate("mainOfficer", "name email")
      .select("name mainOfficer boundary office");
...
// add nagarpalika
const addNagarPalika = async (req, res) => {
  try {
    ...
    const populatedNP = await newNP.populate(
      "mainOfficer",
      "name email"
    );
...
// update a nagarpalika
const updateNagarPalika = async (req, res) => {
  try {
    ...
    if (boundary) {
      np.boundary = boundary;
    }

    if (officerIds) {
      np.officers = officerIds;
    }
...
// delete a nagarpalika
const deleteNagarPalika = async (req, res) => {
  try {
...
// find nagarpalika by location 
const findNagarPalikaByLocation = async (req, res) => {
  try {
    ...
    for (const nagar of nagars) {
      if (isPointInPolygon(point, nagar.boundary)) {
        return res.json({
          nagarId: nagar.nagarId,
          center: nagar.office.coordinates,
        });
      }
    }

    res.status(404).json({
      message: "No NagarPalika found at this location",
    });
  } catch (err) {
    res.status(500).json({
      error: err.message,
...
// get nagarpalika boundary and office location 
const getNagarPalikaBoundary = async (req, res) => {
  try {
    const nagar = await NagarPalika.findOne({
      nagarId: req.params.nagarId,
    });
    ...
    res.json({
      boundary: nagar.boundary,
      location: nagar.office,
    });
  } catch (err) {
    res.status(500).json({
...
// get reports for a specific nagarpalika and department 
const getNagarPalikaReports = async (req, res) => {
  try {
...
//information about a nagarpalika for the home page 
const getHomePageData = async (req, res) => {
  try {
    ...
    trendData = trend.reduce((acc, item) => {
      const label =
