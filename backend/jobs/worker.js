...
const pubClient = createClient({
  url: process.env.UPSTASH_TCP_URL,
});
...
(async () => {
  ...
  reportQueue.process(async (job) => {
    ...
    for (const candidate of nearbyReports) {
      ...
      if (descSim > 0.85) {
        await Report.findByIdAndUpdate(candidate._id, {
          $inc: { priority: 20 },
        });
