...
export default function OfficerInfo({ mode }) {
  ...
  const type = mode === "office" ? "all" : deptId;
  ...
  const addOfficer = () => {
    const { name, email, password } = newOfficer;
    ...
    axios
      ...
      .then((res) => {
        setOfficers((prev) => [...prev, res?.data?.newOfficer]);
        setNewOfficer({
          name: "",
          email: "",
          password: "",
  ...
  useEffect(() => {
    ...
    socket.on("status", (officer) => {
      if (officer && officer.department === selectedDept) {
        setOfficers((prev) =>
          prev.map((o) => (o.name === officer.name ? officer : o))
        );
      }
    });
  ...
  const deleteOfficer = (id) => {
    ...
    axios
      ...
      .then(() => {
        setOfficers((prev) =>
          prev.filter((officer) => officer._id !== id)
        );
      })
    ...
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 max-w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mb-4">
        {mode === "office" ? (
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg font-bold text-blue-700 w-full sm:w-auto focus:outline-none focus:ring-0 focus:border-gray-300"
          >
          ...
          <h2 className="px-3 py-2 border border-gray-300 rounded-lg font-bold text-blue-700 w-full sm:w-auto">
        ...
        {selectedDept !== "all" && (
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#9CFF00] via-[#52E600] to-[#1FCB00] hover:bg-green-600 text-white font-semibold rounded-lg shadow-md transition w-full sm:w-auto"
          >
      ...
      {showAdd && (
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          ...
          <button
            onClick={addOfficer}
            className="flex items-center justify-center gap-1 px-3 py-2 bg-gradient-to-r from-[#9CFF00] via-[#52E600] to-[#1FCB00] hover:bg-green-600 text-white rounded-lg transition col-span-1 sm:col-span-2 lg:col-span-4"
          >
                ...
                >
                  ...
                  <td className="p-3 flex flex-wrap gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-[#FEE403] via-[#FCAE10] to-[#F87918] hover:bg-yellow-600 text-white rounded-lg transition"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-[#FF0018] via-[#D6001F] to-[#A8001A] hover:bg-red-600 text-white rounded-lg transition"
                    >
