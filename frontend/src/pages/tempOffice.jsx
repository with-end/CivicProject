...
export default function HomePage({ mode }) {
  ...
  const variable = mode === "department" ? deptId : mode;
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/nagarpalika/home/${variable}?nagarId=${nagarId}`
        );
  ...
  const handleStatus = async (reportId, prevStatus, department, event) => {
    ...
    try {
      ...
      setStatusCounts((prevCounts) => {
        ...
        if (prevStatus) {
          updated[prevStatus] = Math.max(
            0,
            updated[prevStatus] - 1
          );
        }
      ...
      if (variable === "office") {
        setDeptSummary((prevSummary) => {
          ...
          const updated = {
            ...prevSummary,
            [department]: {
              ...prevSummary[department],
            },
          };

          if (["pending", "inprogress", "approved"].includes(prevStatus)) {
            updated[department].pending = Math.max(
              0,
          ...
          if (prevStatus === "completed") {
            updated[department].resolved = Math.max(
              0,
  ...
  useEffect(() => {
    ...
    socket.on("assigned", (report) => {
      if (
        ...
        (variable === "office" || report.department === variable)
      ) {
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: "Resolved",
        data: Object.values(trendData).map((data) => data.completed),
      ...
      {
        label: "New Issues",
        data: Object.values(trendData).map((data) => data.new),
    ...
    <div className="space-y-6 p-4 sm:p-6 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statusCards.map((card, index) => (
          <div
            key={index}
            className={`${card.color} text-white rounded-xl p-6 shadow-lg flex items-center gap-4`}
          >
            <div>{card.icon}</div>
